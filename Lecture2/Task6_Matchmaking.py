...
# while len(people) > 0:
#     currentMan = people.pop()
#     for p in people:
#         for interest in currentMan['interests']:
#             if interest in p['interests'] and currentMan['gender'] != p['gender']:
#                 print(currentMan['name'] + " и " + p['name'] + " общ интерес: " + interest)

while len(people) > 0:
    currentMan = people.pop()
    for person in people:
        currentManInterests = set(currentMan['interests'])
        pInterests = set(person['interests'])
        if currentManInterests & pInterests and currentMan['gender'] != person['gender']:
            print("{} и {} - интереси: {}"
                  .format(currentMan['name'], person['name'], ", ".join(currentManInterests & pInterests)))
        # break
